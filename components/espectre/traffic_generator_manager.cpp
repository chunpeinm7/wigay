...
void TrafficGeneratorManager::traffic_task_(void* arg) {
  ...
  // Send unicast UDP to gateway. Unicast packets trigger 802.11 MAC-layer ACK
  // responses from the AP, which are the frames CSI is extracted from.
  // Broadcast frames receive no 802.11 ACK and produce no CSI data.
  // ENOMEM during ARP resolution is handled by the 200ms backoff below.
  struct sockaddr_in dest_addr;
  memset(&dest_addr, 0, sizeof(dest_addr));
  dest_addr.sin_family = AF_INET;
  dest_addr.sin_port = htons(9);   // discard port
  dest_addr.sin_addr.s_addr = ip_info.gw.addr; // gateway unicast
  ...
  while (mgr->running_) {
    ...
    if (sent <= 0) {
      if (errno == ENOMEM) {
        // lwIP pool temporarily exhausted - back off and let it recover
        vTaskDelay(pdMS_TO_TICKS(200));
        next_send_time = esp_timer_get_time();  // reset timing after backoff
      } else {
