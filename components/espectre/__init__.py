...
CONFIG_SCHEMA = cv.Schema({
    ...
    # Publish interval in seconds (how often to publish sensor data)
    cv.Optional(CONF_PUBLISH_INTERVAL, default=1.0): cv.float_range(min=0.01, max=3600),
...
async def to_code(config):
    ...
    # Re-enable SPIFFS for calibration storage on newer ESPHome versions
    # that exclude unused ESP-IDF components by default.
    ...
    cg.add(sens.set_force_update(True))  # Always publish even if value unchanged (needed for SSE/web_server)
