...
esp_err_t CalibrationManager::run_calibration_() {
  ...
  esp_task_wdt_reset();  // Feed WDT before heavy computation
  ...
  esp_task_wdt_reset();  // Feed WDT after window scan
  vTaskDelay(1);         // Yield to allow other tasks
  ...
  esp_task_wdt_reset();  // Feed WDT before NBVI calculation
  std::vector<NBVIMetrics> all_metrics(NUM_SUBCARRIERS);
  calculate_nbvi_metrics_(baseline_start, all_metrics);
  
  esp_task_wdt_reset();  // Feed WDT after NBVI calculation
  vTaskDelay(1);         // Yield to allow other tasks
  ...
  esp_task_wdt_reset();  // Feed WDT before sort
...
esp_err_t CalibrationManager::find_baseline_window_(uint16_t* out_window_start, float* out_baseline_variance) {
  ...
  for (uint16_t i = 0; i <= buffer_count_ - window_size_; i += window_step_) {
    // Feed WDT every 50 windows to avoid reset during long calibration
    if (window_idx % 50 == 0) {
      esp_task_wdt_reset();
      vTaskDelay(1);
    }
