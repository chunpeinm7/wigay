...
特徵：Raw Score + Smoothed Score + Delta Raw（3 個）
修正：
  - 先分割原始錄製、再擴增訓練集（防資料洩漏）
  - 驗證集使用未擴增的原始錄製
  - 加入 delta_raw 特徵（運動加速度感）
使用方式：
...
INPUT_SIZE     = 3          # Raw + Smoothed + Delta Raw
SEQ_LEN        = 50
...
def add_delta(seq_2d):
    """(T,2) list/array → (T,3) ndarray；第3欄 = raw[t]-raw[t-1]"""
    arr   = np.array(seq_2d, dtype=np.float32)
    delta = np.zeros((len(arr), 1), dtype=np.float32)
    if len(arr) > 1:
        delta[1:, 0] = arr[1:, 0] - arr[:-1, 0]
    return np.hstack([arr, delta])
# ═══════════════════════════════════════════════
#  3. 前處理
# ═══════════════════════════════════════════════
