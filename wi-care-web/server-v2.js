...
app.use('/gesture-lab/public', express.static(path.join(__dirname, 'gesture-lab', 'public')));
...
const CARE_RECORDS_FILE = path.join(DATA_DIR, 'care-records.json');
...
// 初始化房間開關實體
const ROOM_SWITCHES = [
  { pin: 2, name: '101A', icon: 'mdi:door' },
  { pin: 4, name: '102A', icon: 'mdi:door' },
  { pin: 5, name: '103A', icon: 'mdi:door' },
  { pin: 12, name: '104A', icon: 'mdi:door' },
  { pin: 13, name: '105A', icon: 'mdi:door' },
  { pin: 14, name: '106A', icon: 'mdi:door' },
  { pin: 15, name: '107A', icon: 'mdi:door' },
  { pin: 16, name: '108A', icon: 'mdi:door' },
  { pin: 17, name: '109A', icon: 'mdi:door' },
  { pin: 32, name: '110A', icon: 'mdi:door' },
  { pin: 33, name: '111A', icon: 'mdi:door' },
  { pin: 34, name: '112A', icon: 'mdi:door' },
  { pin: 18, name: '201B', icon: 'mdi:door' },
  { pin: 19, name: '202B', icon: 'mdi:door' },
  { pin: 21, name: '203B', icon: 'mdi:door' },
  { pin: 22, name: '204B', icon: 'mdi:door' },
  { pin: 23, name: '205B', icon: 'mdi:door' },
  { pin: 25, name: '206B', icon: 'mdi:door' },
  { pin: 26, name: '207B', icon: 'mdi:door' },
  { pin: 27, name: '208B', icon: 'mdi:door' },
  { pin: 35, name: '209B', icon: 'mdi:door' },
  { pin: 36, name: '210B', icon: 'mdi:door' },
  { pin: 37, name: '211B', icon: 'mdi:door' },
  { pin: 38, name: '212B', icon: 'mdi:door' },
];
const GPIO_PINS = ROOM_SWITCHES.map(r => r.pin);
ROOM_SWITCHES.forEach(room => {
  const entityId = `switch.gpio_${room.pin}`;
  const existing = entityManager.getEntity(entityId);
  if (!existing) {
    entityManager.registerEntity({
      entity_id: entityId,
      state: 'off',
    ...
    // Always sync friendly_name and icon from config
    entityManager.updateEntityState(entityId, existing.state, {
      friendly_name: room.name,
      icon: room.icon,
      device_class: 'switch'
    });
...
function loadCareRecords() {
  try {
    if (fs.existsSync(CARE_RECORDS_FILE)) {
      return JSON.parse(fs.readFileSync(CARE_RECORDS_FILE, 'utf8'));
    }
  } catch (e) {
    console.error('載入照護紀錄失敗:', e);
  }
  return {};
}

function saveCareRecords(records) {
  try {
    fs.writeFileSync(CARE_RECORDS_FILE, JSON.stringify(records, null, 2));
  } catch (e) {
    console.error('儲存照護紀錄失敗:', e);
  }
}

app.get('/api/care-records/:entity_id', (req, res) => {
  const records = loadCareRecords();
  const entityRecords = records[req.params.entity_id] || [];
  res.json(entityRecords);
});

app.post('/api/care-records/:entity_id', authMiddleware, (req, res) => {
  const { condition, note, handler } = req.body;
  if (!condition) {
    return res.status(400).json({ error: '必須填寫病人狀況' });
  }
  const records = loadCareRecords();
  const entityId = req.params.entity_id;
  if (!records[entityId]) records[entityId] = [];
  ...
  const record = {
    ...
    note: note || '',
    handler: handler || '',
    room_name: entity?.attributes?.friendly_name || entityId,
    resident_name: entity?.attributes?.resident_name || '--'
  };
  records[entityId].unshift(record);
  if (records[entityId].length > 200) records[entityId].length = 200;
  saveCareRecords(records);
